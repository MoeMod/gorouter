...
inline T bswap(T s)
{
	switch (sizeof(T))
	{
	case 2: {auto res = __builtin_bswap16(*(std::uint16_t*)&s); return *(T*)&res; }
	case 4: {auto res = __builtin_bswap32(*(std::uint32_t*)&s); return *(T*)&res; }
	case 8: {auto res = __builtin_bswap64(*(std::uint64_t*)&s); return *(T*)&res; }
	default: return s;
	}
}

#define _LongSwap(x) bswap(x)
